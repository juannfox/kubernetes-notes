...
Kubernetes uses the YAML language for representing resources and objects when working declaratively. These being basically text files with nice syntax makes K8S much easier to comprehend, and is quite aligned with the popular saying "everything is a file on Linux". I expand on this language in the [dedicated YAML section](yaml.md).
...
In the Kubernetes cluster architecture, Worker nodes are those that actually host the workloads in the form of containers, whereas other nodes in the cluster might be Control Plane nodes and be excluded from hosting containers.

See the [dedicated worker node section](nodes.md) for a continuation of this topic.
